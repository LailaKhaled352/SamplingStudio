...
class MainWindow(QMainWindow):
    ...
    # Connect the slider value change to update the frequency label
    ...
    def update_frequency_mode(self):
        ...
        if self.actualRadio.isChecked():
            ...
            self.sampleSlider.setMinimum(1)
            self.sampleSlider.setMaximum(40)  # Corresponding to 0.1 to 4.0 factors of f_max
            self.sampleSlider.setSingleStep(1)
            self.sampleSlider.setValue(1)    # Start at 0.1 * f_max
            self.sample.sampling_mode = 1
             # Update fmax label
            self.fmaxLabel.setText("4 x fmax")
            self.indicatLabel.setText("0.1x fmax")
            self.sample_rate = self.sampleSlider.value()* self.last_actual_mode_value * self.sample.max_freq if self.last_actual_mode_value else 1
    ...
    def update_sampling_frequency(self,signal_data_amplitude):
        ...
        if self.sample.sampling_mode == 1:
           ...
           self.sample_rate = max(self.last_actual_mode_value*(self.sampleSlider.value()/10) * self.sample.max_freq, 1)
           print(f"the sampling rate va;ue at normalized is:{self.sample_rate}")
           self.indicatLabel.setText(f"{self.sampleSlider.value()/10} x fmax")
    ...
    def update_noise(self):
        ...
        self.sample.plot_frequency_domain(self.graph4, self.sample_rate, self.signal.signal_data_time, updated_signal_data_amplitude)
        self.plot_recosntruction()
